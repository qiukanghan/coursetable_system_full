	...
	<view class="container" v-if="isWeekView">
		<view class="week-container">
			<view class="week-column" v-for="week in 7" :key="week">
				...
				:style="{ backgroundColor: item.categoryId.toString() === '1' ? '#e6f3ff' : 'white' }">
...
<script setup>
	...
	const getCourseList = () => {
	  uni.request({
		...
		url: `http://10.27.195.175:8080/api/courses/my?studentId=${studentIdT.value}`,
		method: 'GET',
		success: (res) => {
	...
	const confirmAdd = () => {
		uni.request({
			// url: `http://localhost:3001/courseList?studentId=${studentIdT.value}`,
			url: `http://10.27.195.175:8080/api/courses?studentId=${studentIdT.value}`,
			method: 'POST',
			data: addData.value,
			success: (res) => {
			...
			fail: () => 	uni.showToast({ title: '网络异常，请检查连接', icon: 'none' })
		})
	}
